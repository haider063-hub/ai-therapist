...
1. **Click this button** to start the deployment process:
   [![Deploy with Vercel](https://vercel.com/button)](<https://vercel.com/new/clone?repository-url=https://github.com/echonest-ai-therapy/echonest-ai-therapy&env=BETTER_AUTH_SECRET&env=OPENAI_API_KEY&env=GOOGLE_GENERATIVE_AI_API_KEY&env=ANTHROPIC_API_KEY&envDescription=BETTER_AUTH_SECRET+is+required+(enter+any+secret+value).+At+least+one+LLM+provider+API+key+(OpenAI,+Claude,+or+Google)+is+required,+but+you+can+add+all+of+them.+See+the+link+below+for+details.&envLink=https://github.com/echonest-ai-therapy/echonest-ai-therapy/blob/main/.env.example&demo-title=EchoNest-AI-Therapy&demo-description=An+AI-Powered+Therapy+Chatbot+Built+With+Next.js+and+the+AI+SDK+by+Vercel.&products=[{"type":"integration","protocol":"storage","productSlug":"neon","integrationSlug":"neon"},{"type":"integration","protocol":"storage","productSlug":"upstash-kv","integrationSlug":"upstash"}]>)
...
6. **Enter LLM Provider API Keys in Project Settings.**
   ...
   - Example environment file: [example.env](https://github.com/echonest-ai-therapy/echonest-ai-therapy/blob/main/.env.example)
...
- If you need STDIO support, consider using Render or other hosting platforms.
