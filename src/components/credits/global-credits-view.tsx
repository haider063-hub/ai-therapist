...
interface GlobalCreditsViewProps {
  creditStatus: CreditStatus;
  showUpgradeButton?: boolean;
}

export function GlobalCreditsView({
  creditStatus,
  showUpgradeButton = false,
}: GlobalCreditsViewProps) {
    ...
    <div className="flex items-center gap-2 text-sm">
      ...
      {showUpgradeButton && creditStatus.subscriptionType === "free_trial" && (
        <Button
          size="sm"
          variant="outline"
          className="text-xs h-6 px-2 border-white/30 text-white hover:bg-white/10 ml-2"
